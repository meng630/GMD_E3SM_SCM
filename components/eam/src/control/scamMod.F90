...
!
  ...
  real(r8), public ::      upwp_clasp(1)
  real(r8), public ::      vpwp_clasp(1)
  ...
  real(r8), public ::      wp2rtp_clasp(1)
  real(r8), public ::      wp2thlp_clasp(1)
  real(r8), public ::      wprtpthlp_clasp(1)
...
!
  ...
  ! clasp 
  logical*4, public ::  have_wprtp_clasp
  logical*4, public ::  have_wpthlp_clasp
  logical*4, public ::  have_upwp_clasp
  logical*4, public ::  have_vpwp_clasp
  logical*4, public ::  have_wp2_clasp
  logical*4, public ::  have_thlp2_clasp
  logical*4, public ::  have_rtp2_clasp
  logical*4, public ::  have_rtpthlp_clasp
  logical*4, public ::  have_wprtp2_clasp
  logical*4, public ::  have_wpthlp2_clasp
  logical*4, public ::  have_wp2rtp_clasp
  logical*4, public ::  have_wp2thlp_clasp
  logical*4, public ::  have_wprtpthlp_clasp
...
!   endif
     ...
     if (status .ne. nf90_noerr) then
       ...
       have_wp2_clasp = .true.
       if (isnan(wp2_clasp(1))) have_wp2_clasp = .false.
     endif
     ...
     ! start clasp surface.in 
     status = nf90_inq_varid( ncid, 'lh_clasp', varid   )
     if (status .ne. nf90_noerr) then
       have_wprtp_clasp = .false.
     else
       call wrap_get_vara_realx (ncid,varid,strt4,cnt4,wprtp_clasp)
       have_wprtp_clasp = .true.
       if (isnan(wprtp_clasp(1))) have_wprtp_clasp = .false.
     endif

     status = nf90_inq_varid( ncid, 'sh_clasp', varid   )
     if (status .ne. nf90_noerr) then
       have_wpthlp_clasp = .false.
     else
       call wrap_get_vara_realx (ncid,varid,strt4,cnt4,wpthlp_clasp)
       have_wpthlp_clasp = .true.
       if (isnan(wpthlp_clasp(1))) have_wpthlp_clasp = .false.
     endif

     status = nf90_inq_varid( ncid, 'taux_clasp', varid   )
     if (status .ne. nf90_noerr) then
       have_upwp_clasp = .false.
     else
       call wrap_get_vara_realx (ncid,varid,strt4,cnt4,upwp_clasp)
       have_upwp_clasp = .true.
       if (isnan(upwp_clasp(1))) have_upwp_clasp = .false.
     endif

     status = nf90_inq_varid( ncid, 'tauy_clasp', varid   )
     if (status .ne. nf90_noerr) then
       have_vpwp_clasp = .false.
     else
       call wrap_get_vara_realx (ncid,varid,strt4,cnt4,vpwp_clasp)
       have_vpwp_clasp = .true.
       if (isnan(vpwp_clasp(1))) have_vpwp_clasp = .false.
     endif
     ! end clasp surface.in

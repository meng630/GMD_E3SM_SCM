...
!PMA gustiness output fields
    ...
    ! clasp buoyancy terms 
    call addfld ('WPTHVP_CLUBB2',     (/ 'ilev' /),  'A',     '-', 'wpthvp')
    call addfld ('WP2THVP_CLUBB',     (/ 'ilev' /),  'A',     '-', 'wp2thvp')
    call addfld ('RTPTHVP_CLUBB',     (/ 'ilev' /),  'A',     '-', 'rtpthvp')
    call addfld ('THLPTHVP_CLUBB',     (/ 'ilev' /),  'A',     '-', 'thlpthvp')

    call addfld ('WP2RCP_CLUBB',     (/ 'ilev' /),  'A',     '-', '-')
    call addfld ('RTPRCP_CLUBB',     (/ 'ilev' /),  'A',     '-', '-')
    call addfld ('THLPRCP_CLUBB',     (/ 'ilev' /),  'A',     '-', '-')
    call addfld ('RCP2_CLUBB',     (/ 'ilev' /),  'A',     '-', '-')
    call addfld ('Skw_velocity_CLUBB',     (/ 'ilev' /),  'A',     '-', '-')

    call addfld ('RTP3_CLUBB',     (/ 'ilev' /),  'A',     '-', '-')
    call addfld ('THLP3_CLUBB',     (/ 'ilev' /),  'A',     '-', '-')

    call addfld ('SKW_ZM_CLUBB',    (/ 'ilev' /),  'A',     '-', '-')
    call addfld ('SKW_ZT_CLUBB',    (/ 'ilev' /),  'A',     '-', '-')
...
!-------------------------------------------------------------------------------
   ...
   real(r8) :: wp2rcp_output(pcols,pverp)   
   real(r8) :: rtprcp_output(pcols,pverp) 
   real(r8) :: thlprcp_output(pcols,pverp)   
   real(r8) :: rcp2_output(pcols,pverp)   
   real(r8) :: Skw_velocity_output(pcols,pverp)  

   real(r8) :: wp2rcp(pverp)   
   real(r8) :: rtprcp(pverp)   
   real(r8) :: thlprcp(pverp)   
   real(r8) :: rcp2(pverp)   
   real(r8) :: Skw_velocity(pverp) 

   real(r8) :: rtp3_output(pcols,pverp)  
   real(r8) :: thlp3_output(pcols,pverp)  

   real(r8) :: skw_zm_output(pcols,pverp) 
   real(r8) :: skw_zm(pverp) 

   real(r8) :: skw_zt_output(pcols,pverp) 
   real(r8) :: skw_zt(pverp)    
...
!PMA adds fields for gustiness
   ...
   do i=1,ncol   ! loop over columns
      ...
      ! clasp 
      if (have_wprtp_clasp) wprtp_sfc = wprtp_clasp(1)/(latvap*rho_ds_zm(1))
      if (have_wpthlp_clasp) wpthlp_sfc = wpthlp_clasp(1)/(cpair*rho_ds_zm(1))   
      if (have_upwp_clasp) upwp_sfc = upwp_clasp(1)/rho_ds_zm(1) 
      if (have_vpwp_clasp) vpwp_sfc = vpwp_clasp(1)/rho_ds_zm(1)   
      ...
      do t=1,nadv    ! do needed number of "sub" timesteps for each CAM step
         ...
         call advance_clubb_core_api &
              ...
              wp4, wprtp2, wpthlp2, wp2rtp, wp2thlp, wprtpthlp, &          ! clasp 
              wprtp2_zm, wpthlp2_zm, wp2rtp_zm, wp2thlp_zm, wprtpthlp_zm, &
              skw_zm, skw_zt, &
              wp2rcp, rtprcp, thlprcp, rcp2, Skw_velocity)
...
!                     rtpthlp_in = rtpthlp_in + rtpthlp_mc_out * dtime
      ...
      do k=1,pverp
         ...
         wp2rcp_output(i,k)        = wp2rcp(pverp-k+1)   
         rtprcp_output(i,k)        = rtprcp(pverp-k+1) 
         thlprcp_output(i,k)       = thlprcp(pverp-k+1)   
         rcp2_output(i,k)          = rcp2(pverp-k+1)   
         Skw_velocity_output(i,k)  = Skw_velocity(pverp-k+1)  

         skw_zm_output(i,k)    = skw_zm(pverp-k+1)  
         skw_zt_output(i,k)    = skw_zt(pverp-k+1)  
      ...
      do k=1,pverp
          ...
          ! clasp 
          rtp3_output(i,k)  = rtp3_in(pverp-k+1)
          thlp3_output(i,k)  = thlp3_in(pverp-k+1)
...
!PMA adds gustiness and tpert
   ...
   ! clasp buoyancy terms 
   call outfld( 'WPTHVP_CLUBB2',     wpthvp,            pcols,  lchnk )
   call outfld( 'WP2THVP_CLUBB',     wp2thvp,           pcols,  lchnk )
   call outfld( 'RTPTHVP_CLUBB',     rtpthvp,           pcols,  lchnk )
   call outfld( 'THLPTHVP_CLUBB',   thlpthvp,           pcols,  lchnk )

   call outfld ('WP2RCP_CLUBB',   wp2rcp_output, pcols, lchnk)
   call outfld ('RTPRCP_CLUBB',  rtprcp_output,  pcols, lchnk)
   call outfld ('THLPRCP_CLUBB', thlprcp_output, pcols, lchnk)
   call outfld ('RCP2_CLUBB',    rcp2_output,    pcols, lchnk)
   ...
   call outfld ('SKW_ZM_CLUBB',  skw_zm_output, pcols, lchnk)
   call outfld ('SKW_ZT_CLUBB',  skw_zt_output, pcols, lchnk)

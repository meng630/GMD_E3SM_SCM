...
!-----------------------------------------------------------------------
  ...
  subroutine advance_clubb_core &
             ( l_implemented, dt, fcor, sfc_elevation, hydromet_dim, & ! intent(in)
               ...
               wp4_c, wprtp2_c, wpthlp2_c, wp2rtp_c, wp2thlp_c, wprtpthlp_c, &     ! clasp 
               wprtp2_zmc, wpthlp2_zmc, wp2rtp_zmc, wp2thlp_zmc, wprtpthlp_zmc, &
               skw_zm_c, skw_zt_c, &
               wp2rcp_c, rtprcp_c, thlprcp_c, rcp2_c, Skw_velocity_c)
    ! Description:
    ...
    real( kind = core_rknd ), intent(out), dimension(gr%nz) :: &
      wprtp2_zmc, wpthlp2_zmc, wp2rtp_zmc, wp2thlp_zmc, wprtpthlp_zmc,      &
      wprtp2_c, wpthlp2_c, wp2rtp_c, wp2thlp_c, wprtpthlp_c, wp4_c, &
      skw_zm_c, skw_zt_c, &
      wp2rcp_c, rtprcp_c, thlprcp_c, rcp2_c, Skw_velocity_c
   ...
   real( kind = core_rknd ), dimension(gr%nz) :: &
    ...
    if ( ipdf_call_placement == ipdf_pre_advance_fields &
       ...
       call pdf_closure_driver( dt, hydromet_dim, wprtp,       & ! Intent(in)
                                ...
                                wp2rtp_zm, wprtp2_zm, wp2thlp_zm,    & ! clasp Intent(out)
                                wpthlp2_zm, wprtpthlp_zm)
...
! End Vince Larson's commenting.
      ...
      if ( abs(gr%zm(1)-sfc_elevation) <= abs(gr%zm(1)+sfc_elevation)*eps/2) then
        ...
        call calc_surface_varnce( upwp_sfc, vpwp_sfc, wpthlp_sfc, wprtp_sfc, &      ! intent(in)
                         ...
                         have_wp2_clasp, wp2_clasp(1),                   &
                         have_rtp2_clasp, rtp2_clasp(1),                   &
                         have_thlp2_clasp, thlp2_clasp(1),                   &
                         have_rtpthlp_clasp, rtpthlp_clasp(1),                   &
    ...
    if ( ipdf_call_placement == ipdf_post_advance_fields &
      ...
      wprtp2_zmc    = wprtp2_zm 
      wpthlp2_zmc   = wpthlp2_zm
      wp2rtp_zmc    = wp2rtp_zm
      wp2thlp_zmc   = wp2thlp_zm
      wprtpthlp_zmc = wprtpthlp_zm

...
!      sclr_var_coef = 0.25_core_rknd, & ! This value is made up! - Vince Larson 12 Jul 2005
    ...
    ! clasp 
    real( kind = core_rknd ), intent(in) ::  & 
      wp2_clasp, rtp2_clasp, thlp2_clasp, rtpthlp_clasp 

    logical :: have_wp2_clasp, have_rtp2_clasp, have_thlp2_clasp, have_rtpthlp_clasp
    ...
    if ( l_andre_1978 ) then
       ...
       ! Brian Griffin; February 2, 2008.
       if (have_thlp2_clasp) then 
          thlp2_sfc = thlp2_clasp
       else 
          thlp2_sfc = 0.4_core_rknd * a_const * ( wpthlp_sfc / uf )**2
       end if 
       thlp2_sfc = max( thl_tol**2, thlp2_sfc )
       
       if (have_rtp2_clasp) then 
          rtp2_sfc =rtp2_clasp
       else 
       ...
       if (have_rtpthlp_clasp) then 
           rtpthlp_sfc = rtpthlp_clasp
       else 

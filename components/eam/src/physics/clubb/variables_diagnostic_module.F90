...
! Third order moments
  real( kind = core_rknd ), target, allocatable, dimension(:), public :: & 
    ...
    ! clasp 
    wpthlp2_zm,   & ! w'thl'^2    [m K^2/s]   
    wp2thlp_zm,   & ! w'^2 thl'   [m^2 K/s^2]
    wprtp2_zm,    & ! w'rt'^2     [m kg^2/kg^2]
    wp2rtp_zm,    & ! w'^2rt'     [m^2 kg/kg]
    wprtpthlp_zm    ! w'rt'thl'   [m kg K/kg s]

!$omp threadprivate(wpthlp2, wp2thlp, wprtp2, wp2rtp, &
!$omp   wprtpthlp, wp2rcp, wp3_zm, thlp3, thlp3_zm, rtp3, rtp3_zm, &
!$omp   wpthlp2_zm, wp2thlp_zm, wprtp2_zm, wp2rtp_zm, wprtpthlp_zm)
...
!   --- Allocation ---
    ...
    allocate( wp2rcp(1:nz) )    ! w'^2rc'
     ...
     !clasp 
    allocate( wpthlp2_zm(1:nz) )   ! w'thl'^2
    allocate( wp2thlp_zm(1:nz) )   ! w'^2thl'
    allocate( wprtp2_zm(1:nz) )    ! w'rt'^2
    allocate( wp2rtp_zm(1:nz) )    ! w'^2rt'
    allocate( wprtpthlp_zm(1:nz) ) ! w'rt'thl'
    ...
    !clasp 
    wpthlp2_zm   = 0.0_core_rknd
    wp2thlp_zm   = 0.0_core_rknd
    wprtp2_zm    = 0.0_core_rknd
    wp2rtp_zm    = 0.0_core_rknd
    wprtpthlp_zm = 0.0_core_rknd
...
!------------------------------------------------------------------------
    ...
    if (allocated(sigma_sqd_w_zt)) then
      ...
      !clasp 
      deallocate( wpthlp2_zm )   ! w'thl'^2
      deallocate( wp2thlp_zm )   ! w'^2thl'
      deallocate( wprtp2_zm )    ! w'rt'^2
      deallocate( wp2rtp_zm )    ! w'^2rt'
      deallocate( wprtpthlp_zm ) ! w'rt'thl'

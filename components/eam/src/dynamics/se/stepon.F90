...
subroutine stepon_run3(dtime, cam_out, phys_state, dyn_in, dyn_out)
   ...
   use dyn_comp,    only: dyn_run, TimeLevel, hvcoord
   ...
   do ie=1,nelemd
     call get_temperature(dyn_in%elem(ie),temperature,hvcoord,tl_f)
     ftmp_temp(:,:,:,ie) = temperature !dyn_in%elem(ie)%state%T(:,:,:,tl_f)
   ...
   do ie=1,nelemd
     do k=1,nlev
	   ...
	   forcing_temp(i+(j-1)*np,k) = (temperature(i,j,k) - & !(dyn_in%elem(ie)%state%T(i,j,k,tl_f) - &
	        ftmp_temp(i,j,k,ie))/dtime - dyn_in%elem(ie)%derived%FT(i,j,k)	
           out_temp(i+(j-1)*np,k) = temperature(i,j,k) !dyn_in%elem(ie)%state%T(i,j,k,tl_f)
